...
public class SocksClientForHttp {
	...
	public static void main(String[] args) throws UnknownHostException, IOException {
		String socks5Ip = null;
		int socks5port = 1080;
		String targetUrl = null;
		for (String arg : args) {
			if (arg.startsWith("socks5ip")) {
				socks5Ip = arg.split("=")[1];
			} else if (arg.startsWith("socks5port")) {
				socks5port = Integer.parseInt(arg.split("=")[1]);
			} else if (arg.startsWith("url")) {
				targetUrl = arg.split("=")[1];
			}
		}

		SocksClientForHttp client = new SocksClientForHttp(socks5Ip, socks5port, targetUrl);
		client.connect();
	}
